...
if (isset($_GET['title'])) {
    ...
    // Truy vấn cơ sở dữ liệu để lấy thông tin chi tiết của phim
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<div class='poster'><img src='data:image/jpeg;base64," . base64_encode($row['Poster']) . "' alt='Movie Poster'></div>";
        echo "<div class='details'>";
        echo "<div class='movie-title'><strong>{$row['Title']}</strong></div>";
        echo "<p><strong>Genre:</strong> {$row['Genre']}</p>";
        echo "<p><strong>Writer:</strong> {$row['Director']}</p>";
        echo "<p><strong>Actor:</strong> {$row['Actor']}</p>";
        echo "<p><strong>Plot:</strong> {$row['Plot']}</p>";
        echo "<p><strong>Release Date:</strong> {$row['ReleaseDate']}</p>";
        echo "<p><strong>Language:</strong> {$row['Language']}</p>";
        echo "<p><strong>Rating: </strong> {$row['Ratings']}</p>";
